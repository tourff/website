...
async function loadAdminProducts() {
    ...
    try {
        ...
        if (Array.isArray(products)) {
            list.innerHTML = products.length === 0 ? '<p class="text-xs text-gray-600 uppercase text-center mt-5 uppercase">No Products Found</p>' : '';
            products.forEach(p => {
                list.innerHTML += `
        ...
        list.innerHTML = '<p class="text-red-500 text-[10px] uppercase text-center mt-5">Syncing with server...</p>';
    }
}
